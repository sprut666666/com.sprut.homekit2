...
function createDevice(device, server, group, newPairedDevices)
{
  ...
  for (let key in device.capabilities)
    ...
    switch (device.capabilities[key].id)
    {
      ...
      case 'onoff':
        ...
        if (!group) server.addAccessory(newDevice);
        break;
      ...
      case 'measure_humidity':
        ...
        variableState = HAS.predefined.CurrentRelativeHumidity(characteristicNum++, device.state[key] || 0);
        capabilities.push(variableState);
        arrState[key] = variableState;
      ...
      case 'measure_luminance':
        ...
        variableState = HAS.predefined.CurrentAmbientLightLevel(characteristicNum++, device.state[key] || 0);
        variableState.minValue = - variableState.maxValue;
