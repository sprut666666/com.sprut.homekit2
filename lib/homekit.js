    ...
    switch (device.capabilities[key].id)
    {
      ...
      case 'onoff':
        ...
        if ('measure_power' in device.capabilities)
        {
          ...
          updateStatusOnHKNoNull(Math.round(sDV(device.state.measure_power, 0)), variableState, key, device);
          if (test) testStab(variableState, key, device);
        ...
        if ('meter_power' in device.capabilities)
        {
          ...
          updateStatusOnHKNoNull(Math.round(sDV(device.state.meter_power, 0)), variableState, key, device);
          if (test) testStab(variableState, key, device);
          ...
          if (key in arrState)
          {
            ...
            if (variable != null && variable != undefined)
            {
              switch (device.capabilities[key].id)
              {
                ...
                //////////////////////////////////////////////////////////////////////////////////////////////////////////
                case 'measure_power':
                  updateStatusOnHKNoNull(Math.round(variable), arrState[key], key, device);
                  break;
