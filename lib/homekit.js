...
async function updateStatus(device, variable, status, callback) {
  if (device.available) {
    if (device.state[status] !== variable && variable) {
...
function configServer(homey) {
  ...
  console.log(JSON.stringify(homey), 'success')
  ...
  const manufacturer = HAS.predefined.Manufacturer(2, `Athom ${homey.release}`)
  const model = HAS.predefined.Model(3, `Node ${homey.node_version}`)
...
function createNewDevice(device, id) {
  const newDevice = new HAS.Accessory(id)

  const Identify = HAS.predefined.Identify(1, undefined, (value, callback) => {
    callback(HAS.statusCodes.OK)
  })
  const Manufacturer = HAS.predefined.Manufacturer(2, device.driver.owner_name)
  const Model = HAS.predefined.Model(3, device.driver.id)
  const Name = HAS.predefined.Name(4, device.name)
  const SerialNumber = HAS.predefined.SerialNumber(5, device.id)
  const FirmwareVersion = HAS.predefined.FirmwareRevision(6, '1.0.0')
...
function createDevice(device, server, ungroup) {
  ...
  if (ungroup === undefined) {
    newDevice = createNewDevice(device, server.config.getHASID(device.id))
  }
  ...
  for (const key in device.capabilities) { // eslint-disable-line guard-for-in, no-restricted-syntax
    ...
    switch (device.capabilities[key].id) { // eslint-disable-line default-case
      // ////////////////////////////////////////////////////////////////////////////////////////////////////////
      case 'onoff':
        if (ungroup === true) newDevice = createNewDevice(device, server.config.getHASID(device.id + key))
        ...
        if (ungroup === true) server.addAccessory(newDevice)
        break
      ...
      case 'target_temperature':
        ...
        if ('measure_temperature' in device.capabilities) {
          const temperature = HAS.predefined.CurrentTemperature(characteristicNum++, device.state.measure_temperature || 0)
          capabilities.push(temperature)
        } else {
          const temperature = HAS.predefined.CurrentTemperature(characteristicNum++, targetTemperature2correct(device.state[key] || 20))
          capabilities.push(temperature)
        }
  ...
  device.on('$state', (state) => {
    ...
    if (device.available) {
      for (const key in state) { // eslint-disable-line guard-for-in, no-restricted-syntax
        if (key in arrState) {
          ...
          if (variable) {
            switch (device.capabilities[key].id) { // eslint-disable-line default-case
              ...
              // ////////////////////////////////////////////////////////////////////////////////////////////////////////
              case 'speaker_playing':
                updateStatusOnHKNoNull((variable || false), arrState[key], key, device)
                break
