  ...
  if (device.available)
  {
    if (device.state[status] != variable && variable != null && variable != undefined)
    {
      ...
      }else
      {
        ...
        {
          if (debug) console.log(device.name + ': ' + error, 'error');
...
function updateStatusOnHKNoNull(variable, deviceHK, status, device)
{
  ...
  {
    if (debug) console.log('Update: '+ device.name + ' ' + status + ': ' + deviceHK.value + ' new: ' + variable, "info");
    ...
    switch (device.capabilities[key].id)
    {
      ...
      case 'onoff':
        ...
        if ('dim' in device.capabilities)
        {
          ...
          updateStatusOnHKNoNull(Math.floor(sDV(device.state.dim * 100, 100)), variableState, 'dim', device);
        ...
        if ('light_hue' in device.capabilities)
        {
          ...
          updateStatusOnHKNoNull(Math.floor(sDV(device.state.light_hue * 360, 360)), variableState, 'light_hue', device);
        ...
        if ('light_saturation' in device.capabilities)
        {
          ...
          updateStatusOnHKNoNull(Math.floor(sDV(device.state.light_saturation * 100, 100)), variableState, 'light_saturation', device);
        ...
        if ('light_temperature' in device.capabilities)
        {
          ...
          updateStatusOnHKNoNull(Math.floor(sDV(140 + device.state.light_temperature * 360, 140)), variableState, 'light_temperature', device);
      ...
      case 'dim':
        if (device.class == 'windowcoverings')
        {
          ...
          variableState = HAS.predefined.CurrentPosition(characteristicNum++);
          updateStatusOnHKNoNull(sDV(device.state[key]*100, 100), variableState, key+'dim', device);
          ...
          updateStatusOnHKNoNull(sDV(device.state[key]*100, 100), variableState, key, device);
      ...
      case 'volume_set':
        ...
        updateStatusOnHKNoNull(sDV(device.state[key] * 100 , 10), variableState, key, device);
